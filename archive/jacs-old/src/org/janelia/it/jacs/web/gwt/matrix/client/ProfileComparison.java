...
import org.janelia.it.jacs.web.gwt.common.client.panel.user.jobs.ProfileComparisonJobResultsPanel;
...
public class ProfileComparison extends BaseEntryPoint {
    public static final String TASK_PROFILE_COMPARISON = "ProfileComparisonTask";
    public static final String TASK_ID_PARAM = "taskId";
    public static final String DATASET_PARAM = "dataset";

    private ProfileComparisonJobResultsPanel _ProfileComparisonJobResultsPanel;

    public void onModuleLoad() {
        // Create and fade in the page contents
        clear();
        setBreadcrumb(new Breadcrumb(Constants.PROFILE_COMPARISON_LABEL), Constants.ROOT_PANEL_NAME);
    ...
    private Widget getJobResultsPanel() {
        ...
        _ProfileComparisonJobResultsPanel = new ProfileComparisonJobResultsPanel(TASK_PROFILE_COMPARISON, new JobResultsSelectedListener(),
                new ReRunJobListener(), new String[]{"5", "10", "20"}, 5);
        _ProfileComparisonJobResultsPanel.setJobCompletedListener(new JobCompletedListener());
        resultBox.add(_ProfileComparisonJobResultsPanel);

        // load recent jobs table when browser's done with initial content load
        DeferredCommand.addCommand(new Command() {
            public void execute() {
                _ProfileComparisonJobResultsPanel.first();
            }
        });
    ...
    public Widget getPanel() {
        return new ProfileComparisonPanel(Constants.PROFILE_COMPARISON_LABEL, new JobSubmittedListener());
    }
    ...
    private class JobSubmittedListener implements JobSubmissionListener {
        ...
        public void onSuccess(String jobId) {
            _ProfileComparisonJobResultsPanel.refresh();
        }
    }
