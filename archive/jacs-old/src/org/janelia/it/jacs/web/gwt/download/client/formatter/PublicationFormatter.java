...
public class PublicationFormatter extends BaseDownloadFormatter {
    ...
    public ClickableGrid getCitation(Publication publication, List titleWidgets) {
        ...
        //grid.getRowFormatter().setStyleName(2, "DownloadTabBottomRow");

        Image bulletImage;
        bulletImage = ImageBundleFactory.getControlImageBundle().getSquareBulletUnselectedImage().createImage();
        if (BrowserDetector.isIE())
            bulletImage.setStyleName("DownloadPublicationBulletInTabIE");
        else
            bulletImage.setStyleName("DownloadPublicationBulletInTab");
        ...
        //empty = new HTML("&nbsp;");
        //empty.setStyleName("DownloadEmptyInTab");
        //grid.setWidget(2, 0, empty);
        //
        //Widget downloadLink = new DataFileFormatter().createPaperDownloadLink(publication.getSubjectDocument());
        //grid.setWidget(2, 1, downloadLink);
     ...
     */
    public Panel getDataFiles(VerticalPanel parentPanel,
                              Publication publication,
                              DownloadableDataNodeFilter filter,
                              TreeListener dataFileEventListener) {
        ...
        if (publication.getDataFiles() != null && publication.getDataFiles().size() > 0) {
            ...
            List rootItems =
                    new DataFileFormatter()
                            .addDataFileTrees(parentPanel, publication.getDataFiles(), filter, dataFileEventListener); // , localCollapseExpandListener);
            ...
            //localCollapseExpandListener.setExpander(expander);
        }
    ...
    public SortableTable getAllFiltersTable(List filterValues) {
        ...
        // Must account for previous row count: table may have a header as first row.
    ...
    private String abbreviateAuthors(Publication publication) {
        ...
        if (authors.size() == 0)
            ...
            returnBuffer.append(authors.get(0));
        else if (authors.size() >= 2) {
            returnBuffer.append(authors.get(0)).append(", ").append(authors.get(1));
            if (authors.size() > 2)
                returnBuffer.append(", et al.");
        }

        if (returnBuffer.toString().length() <= ABBREVIATED_TEXT_LENGTH)
            return returnBuffer.toString();
        else
            return returnBuffer.toString().substring(0, ABBREVIATED_TEXT_LENGTH) + "...";
    }
