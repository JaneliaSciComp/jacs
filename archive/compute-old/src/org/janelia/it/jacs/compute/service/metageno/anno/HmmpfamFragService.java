 ...
 * Time: 2:33:06 PM
 */
public class HmmpfamFragService extends MgAnnoBaseService {
    ...
    public void execute(IProcessData processData) throws ServiceException {
        try {
            ...
            if (checkParentTaskForError(parentTask)) {
                logger.info("Parent task has error status - returning");
                return;
            }
            ...
            hmmpfamTask.setParameter(HmmpfamTask.PARAM_pfam_db_node_id, hmmpfamFragDbId);
            if (isSubFileMode()) {
                ...
                hmmpfamTask.setParameter(HmmpfamTask.PARAM_query_node_id, inputFileId.toString());

            }
            else {
                hmmpfamTask.setParameter(HmmpfamTask.PARAM_query_node_id, fileId);
            }
            hmmpfamTask.setParameter(HmmpfamTask.PARAM_project, parentTask.getParameter("project"));
...
//                    // " --threads "+ldhmmpfamThreads +  not necessary for clc
//                    " " + ldhmmpfamFragDb +
...
//            }
            ...
            String hmmpfam2htabStr = "unset PERL5LIB\n" +
                    ...
                    "cat " + resultFile.getAbsolutePath() + " | " + MetaGenoPerlConfig.PERL_BIN_DIR + "/" + hmmpfam2HtabCmd + " > " + resultFile.getAbsolutePath() + ".htab";

            logger.info(hmmpfam2htabStr);
            ...
            // Step 4: parse
            File parseDir = new File(resultFile.getAbsolutePath() + "_hmmpfamFragParseDir");
            parseDir.mkdirs();
            ...
            String changeHtabParsedFullToFragStr = MetaGenoPerlConfig.getCmdPrefix() + changeHtabParsedFullToFragCmd +
                    " -input " + resultFile.getAbsolutePath() + ".htab.parsed";
            ev = sc.execute(changeHtabParsedFullToFragStr, false);
            if (ev != 0) {
                throw new Exception("SystemCall produced non-zero exit value=" + changeHtabParsedFullToFragStr);
            }
            else {
                logger.info("changeHtabParsedFullToFragStr step completed with no errors for parsed file=" + resultFile.getAbsolutePath() + ".htab.parsed");
            }

            // NOTE: We are not adding the frag files to be included in the results. This line must be un-commented to add
            // the frag files to the annotation parser results.
