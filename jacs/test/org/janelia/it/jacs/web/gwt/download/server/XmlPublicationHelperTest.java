...
public class XmlPublicationHelperTest extends TestCase {
    public XmlPublicationHelperTest() {
        super("org.janelia.it.jacs.web.gwt.download.server.XmlPublicationHelperTest");
    }
    ...
    private void testExercisePublicationHelper(boolean testDataFiles) throws Exception {
        ...
        // Configure as spring--inject the dependency.
        ...
        // Use the configured helper, to get a project, using the interface that is
        // exposed to the GWT Entry Point.
        ...
        for (Publication pub : publications) {
            ...
            // NOTE: it is okay for data Files to be empty at lower level.  Not here.
    ...
    private void testDataFiles(List<DownloadableDataNodeImpl> dataFiles) {
        // Assume that failure for null will have been handled by
        // caller.
        if (dataFiles == null)
            return;
        ...
        for (DownloadableDataNode dataFile: dataFiles) {
            ...
            if (dataFile.getChildren() != null &&
                    dataFile.getChildren().size() == 0 &&
                    dataFile.getSize() <= 0) {
            ...
            // Recursion: call for children of this node, as well.
