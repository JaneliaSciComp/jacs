...
//            userName=targetTable.getText(row, USER_LOGIN_COLUMN);
    ...
    private void submitJob() {
        // Validate job name
        // submit the job
        _ticTask = new TicTask();
//        String nodeId = "Test";//naaResultsTable.getValue(naaResultsTable.getSelectedRow().getRowIndex(), 0).toString();
//        _ticTask.setJobName(nodeId);
//        _ticTask.setParameter(NeuronalAssayAnalysisTask.PARAM_inputFile, naaResultMap.get(nodeId).getDatabaseObjectId());
        _submitButton.setEnabled(false);
