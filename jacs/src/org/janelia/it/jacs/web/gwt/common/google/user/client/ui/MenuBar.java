...
/**
 * (This is the standard google MenuBar widget from 1.4.10, updated to allow styles to be set on the menu popup panel,
 * which the standard MenuBar doesn't provide).
 * <p/>
...
public class MenuBar extends Widget implements PopupListener {
    ...
    private String popupStyleName = "gwt-MenuBar-Popup";  /* MAP added */
    ...
    public MenuBar(boolean vertical) {
        ...
        DOM.setElementAttribute(body, "class", "gwt-MenuBar");
    ...
    */
    void doItemAction(final MenuItem item, boolean fireCommand) {
        ...
        popup.setStyleName(popupStyleName); /* MAP added */
        popup.addPopupListener(this);
        shownChildMenu = item.getSubMenu();
        item.getSubMenu().parentMenu = this;

        // Show the popup, ensuring that the menubar's event preview remains on top of the popup's.
        popup.show();
        setPopupPosition(popup, item, vertical); /* MAP - created method, moved call after show()  */
    }

    /**
     * (MAP - created method from code in previous method, for subclass overridability) - sets the position of the
     * MenuItem's popup.
     *
    ...
    private MenuItem findItem(Element hItem) {
        for (MenuItem item1 : items) {
            if (DOM.isOrHasChild(item1.getElement(), hItem)) {
                return item1;
            }
        }
