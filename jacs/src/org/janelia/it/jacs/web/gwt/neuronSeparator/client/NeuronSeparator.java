...
import org.janelia.it.jacs.web.gwt.common.client.panel.user.jobs.NeuronSeparatorJobResultsPanel;
...
public class NeuronSeparator extends BaseEntryPoint {
    public static final String TASK_NEURON_SEPARATOR = "NeuronSeparatorPipelineTask";
    public static final String TASK_ID_PARAM = "taskId";
    public static final String DATASET_PARAM = "dataset";

    private NeuronSeparatorJobResultsPanel _neuronSeparatorJobResultsPanel;

    public void onModuleLoad() {
        // Create and fade in the page contents
        clear();
        setBreadcrumb(new Breadcrumb(Constants.NEURON_SEPARATOR_LABEL), Constants.ROOT_PANEL_NAME);
    ...
    private Widget getJobResultsPanel() {
        ...
        _neuronSeparatorJobResultsPanel = new NeuronSeparatorJobResultsPanel(TASK_NEURON_SEPARATOR, new JobResultsSelectedListener(),
                new ReRunJobListener(), new String[]{"5", "10", "20"}, 5);
        _neuronSeparatorJobResultsPanel.setJobCompletedListener(new JobCompletedListener());
        resultBox.add(_neuronSeparatorJobResultsPanel);

        // load recent jobs table when browser's done with initial content load
        DeferredCommand.addCommand(new Command() {
            public void execute() {
                _neuronSeparatorJobResultsPanel.first();
            }
        });
    ...
    public Widget getPanel() {
        return new NeuronSeparatorPanel(Constants.NEURON_SEPARATOR_LABEL, new JobSubmittedListener());
    }
    ...
    private class JobSubmittedListener implements JobSubmissionListener {
        ...
        public void onSuccess(String jobId) {
            _neuronSeparatorJobResultsPanel.refresh();
        }
    }
