
package org.janelia.it.jacs.web.gwt.common.client.panel.user.node;

import com.google.gwt.core.client.GWT;
import com.google.gwt.user.client.rpc.AsyncCallback;
import com.google.gwt.user.client.rpc.ServiceDefTarget;
import com.google.gwt.user.client.ui.*;
import org.janelia.it.jacs.model.tasks.rnaSeq.UploadRnaSeqReferenceGenomeTask;
import org.janelia.it.jacs.web.gwt.common.client.popup.ErrorPopupPanel;
import org.janelia.it.jacs.web.gwt.common.client.popup.InfoPopupPanel;
import org.janelia.it.jacs.web.gwt.common.client.popup.launcher.PopupCenteredLauncher;
import org.janelia.it.jacs.web.gwt.common.client.service.DataService;
import org.janelia.it.jacs.web.gwt.common.client.service.DataServiceAsync;
import org.janelia.it.jacs.web.gwt.common.client.service.log.Logger;
import org.janelia.it.jacs.web.gwt.common.client.ui.RoundedButton;
import org.janelia.it.jacs.web.gwt.common.client.util.HtmlUtils;

/**
 * Created by IntelliJ IDEA.
 * User: smurphy
 * Date: Aug 20, 2010
 * Time: 4:50:26 PM
 */
public class UserNodeManagementRnaSeqReferenceGenomeNodePopup extends UserNodeManagementNewNodePopup {
    private static Logger _logger = Logger.getLogger("org.janelia.it.jacs.web.gwt.common.client.panel.user.node.UserNodeManagementRnaSeqReferenceGenomeNodePopup");
    private static DataServiceAsync _dataservice = (DataServiceAsync) GWT.create(DataService.class);
    UserNodeManagementRnaSeqReferenceGenomeNodePopup thisPopup;

    static {
        ((ServiceDefTarget) _dataservice).setServiceEntryPoint("data.srv");
    }

    public UserNodeManagementRnaSeqReferenceGenomeNodePopup(String title) {
        super(title);
        thisPopup=this;
    }

    @Override
    protected void populateContent() {
        VerticalPanel panel=new VerticalPanel();

        HorizontalPanel pathMsgPanel = new HorizontalPanel();
        pathMsgPanel.add(HtmlUtils.getHtml("Enter UNIX filesystem path for RnaSeq Reference Genome fasta file", "text"));

        DockPanel _topRow = new DockPanel();
        _topRow.setVerticalAlignment(DockPanel.ALIGN_MIDDLE);
        _topRow.setStyleName("ChooseMessage");
        _topRow.setWidth("100%");
        _topRow.add(pathMsgPanel, DockPanel.WEST);
        _topRow.setCellHorizontalAlignment(pathMsgPanel, DockPanel.ALIGN_LEFT);
        panel.add(_topRow);

        final TextBox unixPathTextBox = new TextBox();
        panel.add(unixPathTextBox);
        panel.add(HtmlUtils.getHtml("&nbsp;", "text"));

        HorizontalPanel nameMsgPanel = new HorizontalPanel();
        nameMsgPanel.add(HtmlUtils.getHtml("Enter a descriptive name for this Reference Genome node", "text"));

        DockPanel _nameRow = new DockPanel();
        _nameRow.setVerticalAlignment(DockPanel.ALIGN_MIDDLE);
        _nameRow.setStyleName("ChooseMessage");
        _nameRow.setWidth("100%");
        _nameRow.add(nameMsgPanel, DockPanel.WEST);
        _nameRow.setCellHorizontalAlignment(nameMsgPanel, DockPanel.ALIGN_LEFT);
        panel.add(_nameRow);

        final TextBox nameTextBox = new TextBox();
        panel.add(nameTextBox);
        panel.add(HtmlUtils.getHtml("&nbsp;", "text"));

        RoundedButton uploadButton = new RoundedButton("Create new node", new ClickListener() {
            public void onClick(Widget widget) {
                _logger.debug("UserNodeManagementRnaSeqReferenceGenomeNodePopup: uploadButton pressed");
                UploadRnaSeqReferenceGenomeTask uploadTask = new UploadRnaSeqReferenceGenomeTask();
                uploadTask.setParameter(UploadRnaSeqReferenceGenomeTask.PARAM_SOURCE_FILE, unixPathTextBox.getText());
                uploadTask.setParameter(UploadRnaSeqReferenceGenomeTask.PARAM_NODE_NAME, nameTextBox.getText());
                uploadTask.setJobName("UploadRnaSeqReferenceGenomeTask");
                _logger.debug("UserNodeManagementRnaSeqReferenceGenomeNodePopup: just before _dataservice.submitJob() taskName="+uploadTask.getTaskName());
                _dataservice.submitJob(uploadTask, new AsyncCallback() {
                    public void onFailure(Throwable throwable) {
                        new PopupCenteredLauncher(new ErrorPopupPanel("An error occurred creating the RnaSeq Reference Genome node.")).showPopup(null);
                        thisPopup.hide();
                    }

                    public void onSuccess(Object o) {
                        new PopupCenteredLauncher(new InfoPopupPanel("Creating node...wait then refresh to access")).showPopup(null);
                        thisPopup.hide();
                    }
                });
                _logger.debug("UserNodeManagementRnaSeqReferenceGenomeNodePopup: just after _dataservice.submitJob()");
            }
        });
        uploadButton.setWidth("130px");
        panel.add(uploadButton);

        panel.add(HtmlUtils.getHtml("&nbsp;", "text"));

        add(panel);
    }
}
