...
set search_path=camera,public;
alter table blast_hit add query_rank integer;
alter table blast_hit add query_hit_rank float4;

create or replace function fnTrigQueryRank() returns trigger as $$
declare
  ignore text;
begin
  if new.event_type='completed' then
    select calculateQryRank(new.task_id,'task') into ignore;
  end if;
  return new;
end $$ language plpgsql;

create trigger trg_task_event_new_completed
  after insert on task_event
  for each row
  execute procedure fnTrigQueryRank();

create or replace function backfillQryRank() returns void as $$
declare
  rec1 record;
  rec2 record;
begin
  for rec1 in select distinct result_node_id from blast_hit order by 1
  loop
    for rec2 in select calculateQryRank(rec1.result_node_id,'node')
    loop
--
    end loop;
  end loop;
