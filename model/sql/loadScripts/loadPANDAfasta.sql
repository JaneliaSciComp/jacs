...
/*
 * this unix command uploads a PANDA .fasta file for processing
 *
./stage_fasta_nuc.sh filename dbname
 *
 * this statement will transform the standard PANDA defline into a near-CAMERA formatted defline:
 *
update stage_fasta_nuc
  set defline=split_part(defline,'|',1)||'_NT_'||split_part(split_part(defline,' ',1),'|',3)||
...
select case seq.obs_flag when true then 'obsolete' when false then 'current' else 'new' end as status, count(*) as num_sequences
  from stage_fasta_nuc nuc left outer join sequence_entity seq using(camera_acc)
...
update stage_fasta_nuc set
    defline=replace(replace(defline,chr(9),' '),chr(8),' '),
    sequence_md5=md5(sequence),
    sequence_length=length(sequence),
    owner_id=60,obs_flag=false,source_id=2,entity_type_code=0,
...
update stage_fasta_nuc f set sequence_id=
...
update stage_fasta_nuc f set sequence_id=
  (select min(sequence_id) from panda_sequence s
   where s.sequence_md5=f.sequence_md5 and s.source_id=2 and s.sequence=f.sequence)
  where sequence_id is null;
update stage_fasta_nuc set sequence_id=-1*nextval('uid_generated_sequence') where sequence_id is null;
-- new duplicated sequences
update stage_fasta_nuc f set sequence_id=
  (select -1*max(sequence_id) from stage_fasta_nuc s
   where s.sequence_md5=f.sequence_md5 and s.sequence=f.sequence)
  where sequence_id<0;
--
-- assign entity_id
-- re-use existing entity if match found
update stage_fasta_nuc f set entity_id=se.entity_id from sequence_entity se where se.camera_acc=f.camera_acc;
update stage_fasta_nuc set entity_id=nextval('uid_generated_sequence') where entity_id is null;
--
-- vacuum/analyze staging tables
vacuum verbose analyze stage_fasta_nuc;
--
-- QA stats
...
insert into panda_sequence(sequence_id,sequence_type_code,sequence_md5,sequence,source_id)
  select sequence_id,2,sequence_md5,sequence,2
  from (select distinct sequence_id,sequence_md5,sequence from stage_fasta_nuc where sequence>'') x
