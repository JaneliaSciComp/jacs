...
test_getTaskStatus.pl - test the getTaskStatus web-service call.

=head1 SYNOPSIS

    USAGE: test_getTaskStatus.pl OPTION LIST HERE

=head1 OPTIONS

test_getPriamStatus.pl - test the getPriamStatus web-service call.

=head1 SYNOPSIS

    USAGE: test_getPriamStatus.pl OPTION LIST HERE
...
=head1 OPTIONS
 ...
 --taskId       The task to check in on.

=head1  DESCRIPTION

This script tests the getTaskStatus web service.  It can be used to check on pretty much any kind of
process that has been submitted already and given a taskId.
