...
test_getBlastDatabaseLocations.pl - test the getBlastDatabaseLocations web-service call.

=head1 SYNOPSIS

    USAGE: test_getBlastDatabaseLocations.pl OPTION LIST HERE
...
=head1 OPTIONS
 ...
 --username     The username under which to search for blast dbs

=head1  DESCRIPTION

This script tests the getBlastDatabaseLocations web service, invoking with the supplied
username, or current user by default.  To see 'pre-installed' databases, supply the --username
argument with 'system'
...
my $program = 'getBlastDatabaseLocations';
...
print SOAP::Lite
    -> service($wsdl_url)
    -> getBlastDatabaseLocations(@options);
