...
test_getMetaGenoCombinedOrfAnnoStatus.pl - test the getMetaGenoCombinedOrfAnnoStatus web-service call.

=head1 SYNOPSIS

    USAGE: test_getMetaGenoCombinedOrfAnnoStatus.pl OPTION LIST HERE
...
=head1 OPTIONS
 ...
 --taskId       VICS job id as returned by runMetaGenoCombinedOrfAnnoStatus

=head1  DESCRIPTION

This script tests the getMetaGenoCombinedOrfAnnoStatus web service.
...
It may also produce an error, such as:

Status Type: error
Status Description: java.lang.Exception: Task 1438306607871035563 finished with non-complete status=error
...
my $program = 'getMetaGenoCombinedOrfAnnoStatus';
...
print SOAP::Lite
    -> service($wsdl_url)
    -> getMetaGenoCombinedOrfAnnoStatus(@options);
