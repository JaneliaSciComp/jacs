...
test_runTophat.pl - test the runTophat web-service call.

=head1 SYNOPSIS

    USAGE: test_runTophat.pl OPTION LIST HERE
...
=head2 VICS parameters
 ...
 --inputFastqReadDirectoryNodeId    VICS id for the input fastq
 --inputReferenceGenomeFastaFileId  VICS id for the reference genome

=head1  DESCRIPTION

This script tests the runTophat web service.
...
my $project = '08020';
my $workSessionId = '';
my $jobName = 'TestTophat';
my $inputFastqReadDirectoryNodeId = '';
my $inputReferenceGenomeFastaFileId = '';
...
my $result = GetOptions("port|p=i"      =>  \$port,
                        ...
                        "jobName=s"                 =>  \$jobName,
                        "inputFastqReadDirectoryNodeId=s"    =>  \$inputFastqReadDirectoryNodeId,
                        "inputReferenceGenomeFastaFileId=s"  =>  \$inputReferenceGenomeFastaFileId,
                        );

$wsdl_url = "http://$server:$port/$wsdl_path/$wsdl_name?wsdl" unless $wsdl_url;

my $program = 'runTophat';
my @options = ( $username,              # username
                '',                     # token
                $project,               # project
                $workSessionId,         # workSessionId
                $jobName,               # jobName
                $inputFastqReadDirectoryNodeId,  # inputFastqReadDirectoryNodeId
                $inputReferenceGenomeFastaFileId,            # inputReferenceGenomeFastaFileId
                );
