...
test_uploadSamFileToSystem.pl - test the uploadSamFileToSystem web-service call.

=head1 SYNOPSIS

    USAGE: test_uploadSamFileToSystem.pl OPTION LIST HERE
...
=head1 OPTIONS
 ...
 --pathToSamFile    The path to the file being uploaded

=head1  DESCRIPTION

This script tests the uploadSamFileToSystem web service.
...
my $result = GetOptions("port|p=i"      =>  \$port,
                        ...
                        "pathToSamFile=s"   =>  \$pathToSamFile,
                        );

$wsdl_url = "http://$server:$port/$wsdl_path/$wsdl_name?wsdl" unless $wsdl_url;

my $program = 'uploadSamFileToSystem';
my @options = ( $username,          # username
                '',                 # token
                $workSessionId,     # workSessionId
                $pathToSamFile,     # pathToSamFile
                );

print "$wsdl_url\n$program\n@options\n";
print SOAP::Lite
    -> service($wsdl_url)
    -> uploadSamFileToSystem(@options);
