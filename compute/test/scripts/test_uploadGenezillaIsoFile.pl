...
test_uploadGenezillaIsoFileToSystem.pl - test the uploadGenezillaIsoFileToSystem web-service call.

=head1 SYNOPSIS

    USAGE: test_uploadGenezillaIsoFileToSystem.pl OPTION LIST HERE
...
=head1 OPTIONS
 ...
 --pathToGenezillaIsoFile    The path to the file being uploaded

=head1  DESCRIPTION

This script tests the uploadGenezillaIsoFileToSystem web service.
...
my $result = GetOptions("port|p=i"      =>  \$port,
                        ...
                        "pathToGenezillaIsoFile=s"   =>  \$pathToGenezillaIsoFile,
                        );

$wsdl_url = "http://$server:$port/$wsdl_path/$wsdl_name?wsdl" unless $wsdl_url;

my $program = 'uploadGenezillaIsoFileToSystem';
my @options = ( $username,          # username
                '',                 # token
                $workSessionId,     # workSessionId
                $pathToGenezillaIsoFile,     # pathToGenezillaIsoFile
                );

print "$wsdl_url\n$program\n@options\n";
print SOAP::Lite
    -> service($wsdl_url)
    -> uploadGenezillaIsoFileToSystem(@options);
