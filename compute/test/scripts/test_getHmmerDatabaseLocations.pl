...
test_getHmmerDatabaseLocations.pl - test the getHmmerDatabaseLocations web-service call.

=head1 SYNOPSIS

    USAGE: test_getHmmerDatabaseLocations.pl OPTION LIST HERE
...
This script tests the getHmmerDatabaseLocations web service, invoking with the username
argument set to some username (default is current) and the token argument set to ''.
...
All dbs owned by the username should be reported on stdout.
...
my $program = 'getHmmerDatabaseLocations';
...
print SOAP::Lite
    -> service($wsdl_url)
    -> getHmmerDatabaseLocations(@options);
