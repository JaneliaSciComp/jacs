...
test_runTargetp.pl - test the runTargetp web-service call.

=head1 SYNOPSIS

    USAGE: test_runTargetp.pl OPTION LIST HERE
...
This script tests the runTargetp web service, invoking with the username
...
my $project = '08100';
my $workSessionId = '';
my $jobName = 'TestTargetp';
my $queryFastaFileNodeId = '1436478961931520171';
my $networkType = '-N';
my $includeCleavage = '1';
my $chloroplastCutoff = '';
my $secretoryCutoff = '';
my $mitochondrialCutoff = '';
my $otherCutoff = '';
...
my $result = GetOptions("port|p=i"      =>  \$port,
                        ...
                        "includeCleavage=s"      =>  \$includeCleavage,
                        "chloroplastCutoff"      =>  \$chloroplastCutoff,
                        "secretoryCutoff"        =>  \$secretoryCutoff,
                        "mitochondrialCutoff"    =>  \$mitochondrialCutoff,
                        "otherCutoff"            =>  \$otherCutoff,

                        );

$wsdl_url = "http://$server:$port/$wsdl_path/$wsdl_name?wsdl" unless $wsdl_url;

my $program = 'runTargetp';
my @options = ( $username,              # username
                ...
                $includeCleavage,       # includeCleavage
                $chloroplastCutoff,     # chloroplastCutoff
                $secretoryCutoff,       # secretoryCutoff
                $mitochondrialCutoff,   # mitochondrialCutoff
                $otherCutoff,           # otherCutoff
