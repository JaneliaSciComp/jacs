...
/**
 * An entity filter which accepts Samples for which the optic lobes do not have neuron separations, 
 * unless the REFRESH parameter is set in the ProcessData.
 * 
 * @author <a href="mailto:rokickik@janelia.hhmi.org">Konrad Rokicki</a>
 */
public class OpticIncompleteSampleEntityFilter implements EntityFilter {

	private static final Logger logger = Logger.getLogger(OpticIncompleteSampleEntityFilter.class);
	...
	public boolean includeEntity(IProcessData processData, Entity entity) {
		...
		for(Entity child : entity.getChildren()) {
			if (child.getEntityType().getName().equals(EntityConstants.TYPE_NEURON_SEPARATOR_PIPELINE_RESULT) 
					&& child.getName().contains("Optic")) {
				logger.info("Skipping optic completed sample "+entity.getName()+" (id="+entity.getId()+")");
				return false;
			}
		}
