...
=item B<--tmpdir, -t>
    ...
    Temporary directory (for clustering algorithm files)
...
sub produce_cluster_output {
    ...
    my $resolver = new Jaccard_coefficient_cluster_resolver($linkscore,$options{'cluster_path'},$options{'tmpdir'});
...
sub check_parameters{
    ...
    $options->{tmpdir} = '/tmp' unless $options->{tmpdir};
