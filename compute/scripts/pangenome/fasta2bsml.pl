...
if ( $options{output_list} ) {
	...
	open( $olist_fh, ">$options{output_list}" )
		|| $logger->logdie("couldn't create output list $options{output_list} : $!");
}
...
if ( ( defined( $options{'genus'} ) ) && ( defined( $options{'species'} ) ) ) {
	...
	if (    ( exists $genome_elem->{'attr'}->{'id'} )
		 && ( defined( $genome_elem->{'attr'}->{'id'} ) ) )
	{
...
sub check_parameters {
	...
	{
		$logger->logdie(
			  "You must specify input with --fasta_input --fasta_list --fasta_file or --fasta_dir");
	}
