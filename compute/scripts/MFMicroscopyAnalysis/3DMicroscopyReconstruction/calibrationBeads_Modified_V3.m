...
%%
function [transformPerSlice,cornerPerSlice,peaksPerSlice]=calibrationBeads(PathName,FileName)
...
for kk=1:length(info)
   stack2D=max(stack2D,double(imread(filenameIm,'Index',kk))); 
end
...
figure
imagesc(stack2Dsmooth);
display('click on the beads in the center slice followed by enter when done')
coordinatesbis = ginput();
coordinatesbis=floor(coordinatesbis);
[m n]=size(coordinatesbis);
numberofbeads=9*m;
...
co1=-3;%correction factor1. For unbinned data, recommend 6. For 512 back illumated Andor camera, it is 0.
co2=-8;%correction factor2. For unbinned data, recommend 3. For 512 back illumated Andor camera, it is -3.
...
aa=7; %half of the width of the region around the spot. For unbinned data, recommend 10.
...
disp(['Saving transformations at ' [PathName FileName '.mat']])
save([PathName FileName(1:end-4) '.mat'],'transformPerSlice','cornerPerSlice','peaksPerSlice');
end