function FISH_QUANT_batch_save_summary_v2(file_name_full,cell_summary,path_name_image,file_name_settings,version)
...
if file_save ~= 0
    ...
    fprintf(fid,'FILE\tCELL\tN_total\tN_thres\n');    
    
    %- Summary for each cell
    for i_cell = 1:size(cell_summary,1)                
        fprintf(fid,'%s\t%s\t%g\t%g\n',cell_summary(i_cell,1).name_image, cell_summary(i_cell,1).cell, cell_summary(i_cell,1).N_total, cell_summary(i_cell,1).N_count);        
    end
end
fclose(fid);
cd(current_dir)
