function [result xdata] = psf_fit_3d_v6(img,xdata,options_fit,flag_struct)
%
...
if isempty(xdata)
    [Xs,Ys,Zs] = meshgrid(0:dim.Y-1,0:dim.X-1,0:dim.Z-1);
...
if strcmp(fit_mode,'sigma_fixed')
    ...
    %- Initial conditions
    x_init = [  par_start.centerx, par_start.centery, par_start.centerz,...
    ...
    result.muX         = x(1);                      % Center X
    result.muY         = x(2);                      % Center Y
    result.muZ         = x(3);                      % Center Z
...
elseif strcmp(fit_mode,'sigma_free_xyz')
    ...
    errordlg('Option not yet implemented!','File psf_fit_3d_v6');    
...
elseif strcmp(fit_mode,'sigma_free_xz')
    ...
    result.muX         = x(3);           % Center X
    result.muY         = x(4);           % Center Y
    result.muZ         = x(5);           % Center Z
