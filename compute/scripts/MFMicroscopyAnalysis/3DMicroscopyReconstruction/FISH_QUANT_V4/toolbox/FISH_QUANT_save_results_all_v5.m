function [file_save path_save] = FISH_QUANT_save_results_all_v5(file_name_full,file_summary,cell_summary,par_microscope,path_name_image,file_name_settings,version,options)

%=== Parameters
flag_only_thresholded = options.flag_only_thresholded;
flag_label            = options.flag_label;
N_ident               = options.N_ident;
...
if file_save ~= 0
    ...
    for i_file = 1:length(file_summary)
        ...
        for i_abs = i_start:i_end
            ...
            name_cell       = cell_summary(i_abs,1).cell;       
            ...
            if not(isempty(spots_fit))
                ...
                elseif flag_label == 2
                    ...
                    file_ident      = name_only(end-N_ident+1:end);    
    ...
    if not(isempty(cell_write_all))
        if      flag_label == 1 
            fprintf(fid,'File\tCell\tPos_Y\tPos_X\tPos_Z\tAMP\tBGD\tRES\tSigmaX\tSigmaY\tSigmaZ\tCent_Y\tCent_X\tCent_Z\tMuY\tMuX\tMuZ\tITERY_det\tY_det\tX_det\tZ_det\tY_min\tY_max\tX_min\tX_max\tZ_min\tZ_max\tSC_det\tSC_det_norm\tTH_det\tTH_fit\n');
            string_write   = ['%s\t%s',repmat('\t%g',1,N_par),'\n'];  

            cell_write_all = sortrows(cell_write_all,[1 2]);    %- Sort by column 1 and then by 2

        elseif  flag_label == 2
            fprintf(fid,'File#\tPos_Y\tPos_X\tPos_Z\tAMP\tBGD\tRES\tSigmaX\tSigmaY\tSigmaZ\tCent_Y\tCent_X\tCent_Z\tMuY\tMuX\tMuZ\tITERY_det\tY_det\tX_det\tZ_det\tY_min\tY_max\tX_min\tX_max\tZ_min\tZ_max\tSC_det\tSC_det_norm\tTH_det\tTH_fit\n');
