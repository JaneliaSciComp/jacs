function [PSF_fit img_PSF] = PSF_3D_Gauss_fit_v4(img_PSF,parameters)
...
%
% v2 March 2,2011
% - Some names of parameters are changed
% - Different call for PSF fit 3D
%
% v3 August, 112011
% - Some names small adjustements
...
%    img_PSF.crop = double(img_PSF.data(img_PSF.max.Y-lp:img_PSF.max.Y+lp,...
 %                                     img_PSF.max.X-lp:img_PSF.max.X+lp,...
%                                      img_PSF.max.Z-lz:img_PSF.max.Z+lz)); 
...
FIT_Result = psf_fit_3d_v6(img_PSF.crop,xdata,options_fit);
...
if flags.output
    ...
    img_PSF_resid_xy = max(PSF_fit.resid,[],3);
    img_PSF_resid_xz = squeeze(max(PSF_fit.resid,[],1));
    img_PSF_resid_yz = squeeze(max(PSF_fit.resid,[],2));  
