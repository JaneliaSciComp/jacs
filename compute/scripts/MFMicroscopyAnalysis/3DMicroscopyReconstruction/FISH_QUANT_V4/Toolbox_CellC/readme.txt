   ...
   environment variable. 
	...
	A. Locate the name of the environment variable to set, using the 
	 ...
	 NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_cellc.sh on UNIX and Mac
...
3. Create packages that contain the following files, based on what
   you are deploying: 
	...
	All Platforms:
		* cellc.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
	Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library archive)  
		* cellc.exe (application created by MATLAB Compiler)
	UNIX:
		* MCRInstaller.zip (MATLAB Component Runtime library 
	 	  archive - manual unzip required)
		* cellc (application created by MATLAB Compiler)
		* run_cellc.sh (shell script that can be run 
	...
	All Platforms:            
		* cellc.dll (shared library, file extension 
		  varies by platform)
		* cellc.h (library header file)
		* cellc.ctf (Component Technology File archive,
		  platform-dependent on target machine)
	...
	* cellc.ctf (Component Technology File archive)	
	* cellc.xml (documentation files)
	* cellc.pdb (program database file - if 
	  DEBUG option selected)
	* cellc.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* cellc.ctf (Component Technology File archive)
	* cellc_1_0.dll (component containing 
	  compiled M-code)
	...
	* cellc.jar (Java archive containing Java 
	  interface to M-code in cellc.ctf)
	* cellc.ctf (Component Technology File archive,
	  platform-dependent on target machine)
