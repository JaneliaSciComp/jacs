function fqbatch(setting_file_fullname, file_name_list)
...
handles.version = 'v0l';
handles.path_imagej = '/Users/mickael/ImageJ';
handles.ij_macro_name = 'FQ_IJ_vis_v3.txt';
...
handles.checkbox_use_filtered = 1;
handles.checkbox_save_filtered = 1;
...
[path_name_settings, file_name_settings, ext_name_settings] = fileparts(setting_file_fullname);
file_name_settings = [file_name_settings, ext_name_settings];

handles = load_settings(handles, file_name_settings, path_name_settings);
% files_add(handles, file_name_list);
handles = process(handles, file_name_list);

save_summary_spots(handles,0);
% =========================================================================
% FILES
% =========================================================================
...
%== Load settings
function settings = load_settings(handles, file_name_settings, path_name_settings)
% [file_name_settings,path_name_settings] = uigetfile({'*.txt'},'Select
% file with settings');
            ...
            if locked
                ...
                par_fields.max_hist;        
                    ...
                    switch par_name
                        ...
                        otherwise
                            disp('Thresholding parameter not defined.');
...
%== Add files
% function files_add(handles, file_name_list)
...
%     set(handles.listbox_files,'String',file_name_list);
...
%== Process all files
function handles = process(handles, file_name_list)
...
if iscell(file_name_list)  
    for i = 1:length(file_name_list)
        [tempPath{i}, tempFilename{i},tempExt{i}] = fileparts(file_name_list{i});
        tempFilenameWExt{i} = [tempFilename{i},tempExt{i}];
    end
    
elseif exist(file_name_list,'file')  %This is a image file list text file
    fid = fopen(file_name_list, 'r');
    fileNum = 0;
    while 1
        tline = fgetl(fid);
        if ~ischar(tline), break, end
        tifFound = strfind(tline,'.tif');
        if ~isempty(tifFound)
            if exist(tline, 'file')
                fileNum = fileNum + 1;
                [tempPath{fileNum}, tempFilename{fileNum},tempExt{fileNum}] = fileparts(tline);
                tempFilenameWExt{fileNum} = [tempFilename{fileNum},tempExt{fileNum}];
            else
                fprintf('The image file %s is non-existent.\n', tline);          
            end
        end
    end
    ...
    disp('The file_name_list should be either a txt list file or a cell arrray of tif files\n');
end


handles.path_name_image = tempPath{1};  %assume all the image files are in the same directory
file_list = tempFilenameWExt;
% path_name = handles.path_name_image;
N_file = length(file_list);


%- (Some) Parameters for detection and fitting
parameters.path_name = tempPath{1};  %assume the files are in the same directory
...
%     end
    ...
    [cell_prop par_microscope file_name_image file_name_image_filtered] = spot_detect_fit_batch(handles,parameters);  
...
handles = results_summarize(handles);
if not(isempty(handles.spots_fit_all))
    handles = results_analyse(handles);
%     handles = pop_up_threshold_Callback(eventdata, handles);
%     handles = button_threshold_Callback(eventdata, handles);
%     guidata(hObject, handles); 
else
...
function handles = results_summarize(handles)
...
function handles = results_analyse(handles)
...
function save_summary_spots(handles,flag_threshold)
...
if not(isfield(handles,'file_name_settings_new'))
    ...
    %- Save results
    % if( ~ isempty(userValue))
    %     name_settings  = userValue;
    
    handles.file_name_settings_new  = FISH_QUANT_save_settings_batch(fullfile(handles.path_name_image,name_settings),handles);
