...
sub get_best_evidence {
	...
	while ( @fetchRow{@$query_key_list} = $sth->fetchrow_array() ) {
		...
		if ( $coverage eq 'blast' ) {
			$row{coverage}
				= &calculate_blast_coverage( $row{alignment_length}, $row{query_length},$row{subject_length} );
		}
...
sub create_annotation {
    ...
    $orf->{ANNOTATION}->{taxon} = &get_taxon_info($orf);
...
sub get_equivalog_tigrfam_hmm {
	...
	#print STDERR Dumper($orf->{'PFAM/TIGRFAM_HMM'});exit;
...
sub get_iprscan_results{
    ...
    if ( exists $orf->{'IPRSCAN'}->{hits} ) {
	my $sorted_ref = &rank_iprscan_hits(\@{$orf->{'IPRSCAN'}->{hits}});
	foreach my $hit(@$sorted_ref) {

        $$name      = $hit->{'db_member_desc'};
	    $$reference = "$hit->{'analysis_method'}:$hit->{'db_member_id'}";
...
sub rank_iprscan_hits{
    my($ref)=@_;
    my $evalue ="";
    my @sort = sort{$a->{'evalue'} <=> $b->{'evalue'}}@$ref;
...
sub get_taxon_info {
    ...
    if ( exists $orf->{ALLGROUP_PEP}->{hits} ) {
        ...
        foreach my $hit ( @{ $orf->{ALLGROUP_PEP}->{hits} } ) {
            ...
            if ( $hit->{subject_definition} =~ /taxon:(\d+)/ ) {
                $taxon{$1}++;
            }
        
        }
        my @taxons = sort {$taxon{$a} <=> $taxon{$b}} keys %taxon;
        return \@taxons;
...
sub get_query_print_key_list {
	...
	if ( $job_name eq 'PEPSTATS' ) {
	    ...
	    }elsif($job_name eq "IPRSCAN"){

		@query_key_list = qw( query_id query_length analysis_method db_member_id
			evalue job_name dm_start dm_end status interpro_id interpro_desc db_member_desc
			job_name);
		@print_key_list = qw( db_member_id analysis_method interpro_desc db_member_desc query_length evalue);
		$table    = "compute_iprscan";
		$coverage = 'interpro';
	    }
...
sub calculate_blast_coverage {
	...
	#         query_length
        ...
        ## made cahnges for fixing the coverage problem Nikhat
	my ( $alignment_length, $query_length,$subject_length) = @_;

	#print "$alignment_length, $query_length,$subject_length\n";
	...
	my $coverage_2 = $alignment_length /$subject_length;
	#print "$coverage_2\n";
...
sub calculate_hmm_coverage {
	...
	#print "HMM COVERAGE : $hmm_beg, $hmm_end, $hmm_len \n";
...
# return the dataset name from the db.

    my ($dbh) = @_;

    my $dataset_name = '';
    my $query = "select dataset_name from dataset where dataset_name != 'no dataset'";

    my $sth = $dbh->prepare($query) or die "$DBI::errstr\n";
    $sth->execute();
    $sth->bind_columns(\$dataset_name);
    $sth->fetch();
    return $dataset_name;
...
sub get_evidence_query {
	...
	$query .= " FROM $table c WHERE job_id = $job_id and query_seq_id= $query_id and evalue != 'NA'";
...
sub print_evidence_results {
    ...
    # silence errors stemming from no orf_ids being found:
    return unless ($orf_id);
	...
	foreach my $job_name ( qw(CDD_RPS ALLGROUP_PEP ACLAME_PEP SANGER_PEP
						   ...
						   ACLAME_HMM PRIAM_GENE_RPS PEPSTATS TMHMM SIGNALP IPRSCAN)
		)
	{
...
sub print_annotation_results {
    ...
    # silence errors from no orf being found:
    return unless ($orf_id);
	...
	foreach my $job_name (qw(SIGNALP TMHMM PEPSTATS)) {
		...
		# for all jobs with at least 1 hit
		if ( exists $orf->{$orf_id}->{$job_name} && 
			defined @{ $orf->{$orf_id}->{$job_name}->{hits} }) {
				print "$job_name\t";
...
sub print_metarep_results {
=head print_metarep_results

    METAREP needs input in the following format:
    Column  FieldID         Description
    ------  ------------    -----------
        1   peptide_id      Unique peptide ID
        2   library_id      Library ID
        3   com_name        Functional description
        4   com_name_src    Source of functional description assignment
        5   go_id           Gene Ontology ID
        6   go_src          Source of Gene Ontology assignment
        7   ec_id           Enzyme Commission ID
        8   ec_src          Source of Enzyme Commission ID
        9   hmm_id          Hidden Markov Model hits (e.g. PFAM/TIGRFAM)
       10   blast_taxon     NCBI taxonomy ID
       11   blast_evalue    BLAST E-Value
       12   blast_pid       BLAST percent Identity
       13   blast_cov       BLAST sequence coverage of shortest sequence
       14   filter          Any filter tag (categorical variable)
=cut

	my ( $orf, $mfh, $library_name ) = @_;
	...
	print "$orf_id\t$library_name\t";                       # 1 & 2 (peptide_id & library_id)
	print "$orf->{$orf_id}->{ANNOTATION}->{name}\t";        # 3     com_name
	print "$orf->{$orf_id}->{ANNOTATION}->{name_ev}\t";     # 4     com_name_src
	...
	if ( defined $orf->{$orf_id}->{ANNOTATION}->{go_terms} ) {
		...
		print join( " || ", @go_term );       # 5   go_id
		print "\t";
		print join( " || ", @go_evidence );   # 6   go_src
		print "\t";
	...
	# print EC
	if ( defined $orf->{$orf_id}->{ANNOTATION}->{ec_number_ev} ) {
		print "$orf->{$orf_id}->{ANNOTATION}->{ec_number}\t";       # 7     ec_id
		print "$orf->{$orf_id}->{ANNOTATION}->{ec_number_ev}\t";    # 8     ec_src
	}
	else {
		print "\t\t";
	}
    ...
    my %hmms;
	if ( defined $orf->{$orf_id}->{'PFAM/TIGRFAM_HMM'}->{all_hits} ) {
        my @hits = @{$orf->{$orf_id}->{'PFAM/TIGRFAM_HMM'}->{all_hits}};
        foreach my $hit (@hits) {
            $hmms{$hit}++;
        }
		print join( " || ", keys %hmms );   # 9 hmm_id
		print "\t";
	}
	else {
		print "\t";
    ...
    # print taxonomy
    if ( defined $orf->{$orf_id}->{ANNOTATION}->{taxon} &&
                 $orf->{$orf_id}->{ANNOTATION}->{taxon} ) {
        my @taxons = @{$orf->{$orf_id}->{ANNOTATION}->{taxon}};
        print join (" || ", @taxons); # 10    blast_taxon
    }
    print "\t";
    

	# print best blast info
	if ( defined $orf->{$orf_id}->{ALLGROUP_PEP}->{hits} ) {

        my $hits = $orf->{$orf_id}->{ALLGROUP_PEP}->{hits};

        # want lowest evalue, highest pid & cov
        my ($evalue, $pid, $cov);
        foreach my $hit (@$hits) {
            my ($cur_evalue, $cur_pid, $cur_cov) = @{$hit}{('evalue','pct_identity','coverage')};
            $evalue = (defined $evalue) ? min($cur_evalue,  $evalue ) : $cur_evalue;
            $pid    = (defined $pid)    ? max($cur_pid,     $pid    ) : $cur_pid;
            $cov    = (defined $cov)    ? max($cur_cov,     $cov    ) : $cur_cov;
        }

		print "$evalue\t";  # 11    blast_evalue
        print "$pid\t";     # 12    blast_pid
        print "$cov\t";     # 13    blast_cov
