...
#	my $numInserted = 0;
#	foreach my $row(@$data) {
#		my $insert = &executeSQL($dbh,$insertSQL,@$row);
#		if ( ! defined $insert ) {
#	    	$errorMessage = "bulkInsertData after $numInserted inserts: " . $errorMessage; 
#	    	return undef;
#		} else {
#			$numInserted++;
#		}
#	}
	...
	if ( ! defined $numInserted ) {
		$errorMessage = "bulkInsertData insert failed: " . &bulkErrorMessage(\@rowStatus);
		return undef;
...
# will implement error handling later
	...
	my ( $yyyyy, $mm, $dd, $hr24, $mi, $ss ) = &parseDate($value,$fromRDBMS);
	if ( !defined $yyyy ) {
