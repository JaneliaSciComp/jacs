...
# add fasta
my $dataset = &addNode( $dbh, $name, $description, $node_id, $node_owner, "seq" );
$dbh->disconnect;
if ( !defined $dataset ) {
	die "\nCould not add import VICS blast db: " . $errorMessage . "\n";	
}
print "\nDataset imported:\n"
...
sub initialize {
	...
	if ( $opt_h ) {
		print
"
This script imports a blast DB from VICS into a config db for use as a SUBJECT dataset.

usage: ./importBlastDB.pl -D myConfig.db -I 1306784010227155354 -N \"NCBI NINT\" -d \"Non-identical nucleotide sequences from NCBI.\"
...
";
	...
	if ( !$opt_I ) {
		die "\nYou must specify the VICS blast db node id (-I).\n";
	}
