...
PACKAGE_WINDOWS_DIR="$STAGING_DIR/JaneliaWorkstation_windows_${JWVER}"

STAGING_PACKAGE_MAC_DIR="$EXE_DIR/JaneliaWorkstation_${JWVER}"
STAGING_PACKAGE_LINUX_DIR="$EXE_DIR/JaneliaWorkstation_linux_${JWVER}"
STAGING_PACKAGE_WINDOWS_DIR="$EXE_DIR/JaneliaWorkstation_windows_${JWVER}"

echo "Building Janelia Workstation version $JWVER (Part 1)"
...
if [ $BUILD_VAA3D == 1 ]; then
    ...
    svn $SVN_OPTIONS co https://subversion.int.janelia.org/ScientificComputing/Projects/BrainAligner/tags/JaneliaWorkstation_${JWVER}/jba $VAA3D_COMPILE_REDHAT_DIR/jba
    svn $SVN_OPTIONS co https://subversion.int.janelia.org/ScientificComputing/Projects/BrainAligner/tags/JaneliaWorkstation_${JWVER}/flybrainAlign/ireg $VAA3D_COMPILE_REDHAT_DIR/released_plugins/v3d_plugins/ireg
    svn $SVN_OPTIONS co https://subversion.int.janelia.org/ScientificComputing/Projects/BrainAligner/tags/JaneliaWorkstation_${JWVER}/imath $VAA3D_COMPILE_REDHAT_DIR/released_plugins/v3d_plugins/imath
...
if [ $BUILD_JACS == 1 ]; then
    ...
    echo "Changing versions to ${JWVER}"
    sed "s/console.versionNumber=DEV/console.versionNumber=${JWVER}/g" console/src/main/java/console.properties > tmp
    mv tmp console/src/main/java/console.properties
    sed "s/jacs.version=DEV/jacs.version=$JWVER/g" shared/conf/jacs.properties > tmp
    mv tmp shared/conf/jacs.properties
...
################################################################
if [[ $SERVER == jacs ]] && [[ $BUILD_JANELIAWORKSTATION == 1 ]]; then
...
if [ $RUN_PART2 == 1 ]; then
    ...
    ssh $SSH_OPTIONS $MAC_EXECUTOR_HOST "sh $SCRIPT_DIR/build_jacs_part2_mac.sh $JWVER $SERVER $PART2_BUILD_VAA3D $PART2_BUILD_JANELIAWORKSTATION"
fi
